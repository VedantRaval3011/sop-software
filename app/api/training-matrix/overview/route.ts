...
function monthCountsFromSchedule(excel: { excelCodes: Set<string>; sopMonthMap: Record<string, string> } | null): Record<string, number> {
  ...
  for (const base of excel.excelCodes) {
    const mo = smm[base];
    if (!mo) continue;
    // One SOP = one month slot (primary month). Multi-month entries must not inflate the strip sum.
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (entry) {
      ...
      // Hard invalidation (computedAt=0) happens after Manage SOP saves — recompute
      // synchronously so assigned-SOP counts on the training matrix match manage-sops.
      if (!entry.computedAt) {
        const payload = await computeOverviewPayload(true);
        await setTrainingMatrixCached(payload);
        return NextResponse.json(payload);
      }
      // Soft TTL expiry: serve cached payload instantly, refresh in background (SWR).
...
async function computeOverviewPayload(forceFresh: boolean) {
    ...
    // Obsolete / non-registry Excel codes (e.g. QAIO38) must not appear in the matrix.
    ...
    for (const dept of departments) {
      ...
      // Month counts: assigned SOPs only (excelCodes), scheduled via sopMonthMap.
      // Must not union DB codes — that inflates totals vs dept cards / manage-sops.
