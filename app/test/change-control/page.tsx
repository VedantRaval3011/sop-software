...
export default function ChangeControlTestPage() {
  ...
  const generate = async () => {
    ...
    setError('');
    try {
      const res = await fetch('/api/test/change-control', {
        method: 'POST',
    ...
    try {
      await fetch('/api/mcq-tests', {
        ...
        body: JSON.stringify({
          ...
          testName: 'Change Control Training Test',
        }),
      });
    } catch {
      /* non-fatal */
  ...
  if (step === 'selection') {
    return (
      <QuestionBasisSelection
        ...
        title="Change Control Test — Choose Question Basis"
      />
    );
  }
    ...
    <div className="min-h-screen bg-slate-900 p-4 md:p-8">
      <div className="max-w-3xl mx-auto">
        ...
        <h2 className="text-2xl font-bold text-white mb-6">
          {step === 'difficulty' ? 'Select Difficulty' : 'Configure Change Control Test'}
        </h2>
          ...
          <div className="space-y-5">
              ...
              <>
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-white/10 border border-white/20 rounded-xl pl-9 pr-4 py-2.5 text-white placeholder-slate-400 text-sm focus:outline-none focus:border-orange-400"
                  />
                </div>
                      ...
                      return (
                        <button
                          ...
                          className={`w-full text-left px-4 py-3 rounded-xl border text-sm transition-all ${
                            selected
                              ? 'bg-orange-600/20 border-orange-400 text-orange-200'
            ...
            <div className="bg-white/5 border border-white/10 rounded-2xl p-5">
              <div className="flex justify-between items-center mb-2">
                ...
                <span className="text-orange-300 font-bold">{questionCount}</span>
              </div>
              <input
            ...
            <button
              ...
              className="w-full py-3.5 rounded-2xl bg-orange-600 hover:bg-orange-500 text-white font-semibold disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center justify-center gap-2"
            >
              {loading ? (
