...
export default function RegularTestPage() {
  ...
  // selectedBankIds holds actual MCQBank _id values (from registry entry.banks[*].id)
  ...
  const toggleEntry = (entry: RegistryEntry) => {
    ...
    setSelectedBankIds((prev) => {
      ...
      if (alreadySelected) return prev.filter((id) => !ids.includes(id));
      return [...prev, ...ids];
    });
  };
  ...
  const generate = async () => {
    ...
    setError('');
    try {
      const res = await fetch('/api/test/regular', {
        method: 'POST',
    ...
    try {
      await fetch('/api/mcq-tests', {
        ...
        body: JSON.stringify({
          ...
          testName: 'Regular Training Test',
        }),
      });
    } catch {
      /* non-fatal */
  ...
  if (step === 'selection') {
    return (
      <QuestionBasisSelection
        ...
        title="Regular Training Test — Choose Question Basis"
      />
    );
  }
    ...
    <div className="min-h-screen bg-slate-900 p-4 md:p-8">
      <div className="max-w-3xl mx-auto">
        ...
        <h2 className="text-2xl font-bold text-white mb-6">
          {step === 'difficulty' ? 'Select Difficulty' : 'Configure Regular Training Test'}
        </h2>
          ...
          <div className="space-y-5">
                ...
                <div className="flex gap-3">
                  <div className="relative flex-1">
                    ...
                    <input
                      ...
                      className="w-full bg-white/10 border border-white/20 rounded-xl pl-9 pr-4 py-2.5 text-white placeholder-slate-400 text-sm focus:outline-none focus:border-emerald-400"
                    />
                  </div>
                  <select
                    value={selectedDept}
                    onChange={(e) => setSelectedDept(e.target.value)}
                    className="bg-white/10 border border-white/20 rounded-xl px-3 py-2.5 text-white text-sm focus:outline-none focus:border-emerald-400"
                  >
            ...
            <div className="bg-white/5 border border-white/10 rounded-2xl p-5">
              <div className="flex justify-between items-center mb-2">
                ...
                <span className="text-emerald-300 font-bold">{questionCount}</span>
              </div>
              <input
                type="range"
                min={5}
            ...
            <button
              ...
              className="w-full py-3.5 rounded-2xl bg-linear-to-r from-emerald-600 to-teal-600 text-white font-semibold hover:from-emerald-500 hover:to-teal-500 disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center justify-center gap-2"
            >
              {loading ? (
                <>
